...
import 'package:elite/custom_model/movie_model.dart' as m;
...
import 'package:elite/model/movie.dart' as model;
...
class VideoDescrptionScreen extends StatefulWidget {
  const VideoDescrptionScreen({super.key, required this.model, required this.isTrailer, this.lastPosition});
  final m.Movie model;
  final bool isTrailer;
  final int? lastPosition;

  @override
  State<VideoDescrptionScreen> createState() => _VideoDescrptionScreenState();
}

class _VideoDescrptionScreenState extends State<VideoDescrptionScreen> with Utility {
  double _downloadProgress = 0.0;
  bool _isDownloading = false;
  bool _isDownloaded = false;
  PostContinueWatchingCubit? _watchingCubit;

  Future<void> _downloadVideo({required model.Movie movie}) async {
    try {
      setState(() {
        _isDownloading = true;
      });
  ...
  void _checkIfDownloaded() {
    ...
    _isDownloaded = HiveDb.isMovieDownloaded(id);
    setState(() {});
  }
                                      ...
                                      if (!_isDownloading && !_isDownloaded) {
                                        _downloadVideo(
                                          movie: model.Movie(
                                            ...
                                            reportCount: widget.model.reportCount,
